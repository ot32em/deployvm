...
def unicast_download(url, save_dir='./', chunk_size=10*1024*1024, debug=False): # 10 MB chunk size for default setting
    ...
    try:
        ...
        return False
    except ul.HTTPError as r:
        print("HTTP Error: %s [%s]" % (r.reason, url))
        return False
    except ul.URLError as r:
        print("HTTP Error: %s [%s]" % (r.reason, url))
        return False
